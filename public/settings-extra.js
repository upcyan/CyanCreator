...
export function settingsExtras(state,search='',sort='time-desc',editName=null){
  ...
  return `<section class="panel"><h2>密钥保险库</h2><p class="hint">密钥使用 Windows 当前账户加密。保存后不回显，不写入浏览器存储或项目文件。密钥引用名可供多个模型共用；与已有密钥同名时，保存会更新其内容。</p><div class="warning" id="secret-missing" ${missing.length?'':'hidden'}>⚠ 有 ${missing.length} 个引用密钥尚未录入，在各待补全行粘贴 KEY 后，点下方「加密保存密钥」一次保存：</div><div id="secret-pending">${secretPendingHtml(missing)}</div><div class="cols"><label>引用名称<input id="secret-name" data-transient placeholder="其他新密钥的引用名，如 ARK_API_KEY" autocomplete="off"></label><label>API Key / Secret Key<input id="secret-value" data-transient type="password" autocomplete="new-password" placeholder="仅输入需要保存的新密钥"></label></div><button data-action="secret-save">加密保存密钥</button><div class="secret-toolbar"><h3 class="secret-title">已保存密钥</h3><input id="secret-search" data-transient placeholder="搜索密钥名称…" value="${search}"><select id="secret-sort" data-transient><option value="time-desc" ${sort==='time-desc'?'selected':''}>添加时间 从新到旧</option><option value="time-asc" ${sort==='time-asc'?'selected':''}>添加时间 从早到晚</option><option value="asc" ${sort==='asc'?'selected':''}>名称 A→Z</option><option value="desc" ${sort==='desc'?'selected':''}>名称 Z→A</option></select><span class="hint" id="secret-count"></span></div><div id="secret-rows"></div><p class="hint">同一系统账户下运行的恶意软件仍可能访问解密能力；不要共享系统账户或保险库备份。环境变量仍可使用。</p></section>`;}
