...
当前采用上述手动更新流程，尚未提供页面内自动拉取、应用更新和重启按钮。需要回退时停止服务，切回已知正常提交，执行 `npm ci`，并恢复与该版本配套的数据备份。
...
2026-09-15 共 19 项测试通过；覆盖原生进程协议、取消、临时文件清理、镜头参数隔离、批量提交原子性、版本选择、静态资源缓存头及原生依赖指纹更新。真实 FFmpeg 输出 1280×720，约 1.77 秒，含音轨。模型协议与部署状态回归测试使用明确的 fixture。
