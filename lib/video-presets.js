...
export function wanPreset() {
  return {provider:'comfy',baseUrl:'http://127.0.0.1:8188',model:'Wan2.1-T2V-1.3B',keyEnv:'',profile:'Wan 2.1 · 1.3B FP16 / 文本编码器 FP8',outputNode:'11',durationMode:'wan',
    params:{seed:42,steps:20,cfg:6,width:832,height:480,frames:81},
    bindings:{prompt:{node:'4',input:'text'},seed:{node:'8',input:'seed'},steps:{node:'8',input:'steps'},cfg:{node:'8',input:'cfg'},width:{node:'7',input:'width'},height:{node:'7',input:'height'},frames:{node:'7',input:'length'}},
    workflow:{
...
export function videoPreset(id) {const p=videoPresets.find(p=>p.id===id);if(!p)throw new Error('未知视频模型方案');return {...p.create(),catalogId:id};}
