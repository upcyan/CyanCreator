...
export async function assistText(p,config,options,signal,onProgress){
  ...
  const shape=stage==='characters'?{characters:[{name:'姓名',cast:'阵容（主角团/配角/反派阵营/路人）',appearance:'外观',personality:'初始性格',motivation:'初始动机',voice:'声音特征',notes:'备注'}]}:stage==='outline'?{logline:'一句话故事',beats:[{title:'标题',summary:'内容'}]}:{scenes:[{title:'场景',action:'动作',dialogue:'台词',shots:[{prompt:'画面',duration:5}]}]};
  const context={brief:p.brief,bible:p.bible,characterRelations:p.characterRelations||[],characters:p.characters,worldbook:p.worldbook,outline:p.outline,script:stage==='script'?p.script:undefined,chapter:p.episodes?.flatMap(e=>e.chapters).find(c=>c.id===p.activeChapterId)?.title,instruction:options.instruction,mode:options.mode};
