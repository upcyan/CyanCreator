...
import {buildWorkflow, requireValue} from './core.js';
...
export function editShot(original,input,config){
  ...
  requireValue(input.firstFrameId===undefined||input.firstFrameId===null||input.firstFrameId===''||(typeof input.firstFrameId==='string'&&input.firstFrameId.length<=100),'首帧图引用无效');
  requireValue(input.lastFrameId===undefined||input.lastFrameId===null||input.lastFrameId===''||(typeof input.lastFrameId==='string'&&input.lastFrameId.length<=100),'尾帧图引用无效');
  ...
  if(input.firstFrameId)shot.firstFrameId=input.firstFrameId;else delete shot.firstFrameId;
  if(input.lastFrameId)shot.lastFrameId=input.lastFrameId;else delete shot.lastFrameId;
