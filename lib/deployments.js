...
export class Deployments {
  constructor(state,data,persist,deps={}) {
    ...
    this.download=deps.download||downloadFile;this.check=deps.check||preflight;this.controllers=new Map();this.pumping=false;
  ...
  async pump() {
    ...
    try {let j;while((j=this.state.deployments.findLast(j=>j.status==='queued'))){
      ...
      try {
        const entry=catalogEntry(j.modelId),root=entry.kind==='text'?path.join(this.data,'models'):j.config.comfyModels;
        let last=0;
        const progress=p=>{j.progress=p;if(Date.now()-last>500){last=Date.now();this.persist();}};
        if(entry.kind==='video'){progress({phase:'starting',file:'ComfyUI'});await this.runtime.ensureVideo(j.config,signal);}
        const files=[];
