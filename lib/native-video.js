...
export function runNativeCommand(exe,args,{signal,input,onEvent=()=>{}}={}){
  return new Promise((resolve,reject)=>{
    ...
    child.on('close',code=>{if(signal?.aborted)return reject(signal.reason);code===0?resolve(result||{}):reject(new Error(tail||`独立推理进程退出 (${code})`));});
...
export class NativeVideo {
  ...
  async environment(config,signal){
    const python=this.python(config);if(!existsSync(python))throw new Error('原生环境尚未安装，请先在模型库安装独立运行环境');
  ...
  async generate(config,deployment,prompt,signal,onEvent){
    validateNative(config);if(this.installing||this.generating)throw new Error('原生运行环境忙，请稍后重试');
