...
export class SpeechRuntime{
  ...
  async prepare(config,signal,progress){
    ...
    if(!existsSync(path.join(this.root,'ready-1.8.0'))){progress({message:'安装 Piper 1.8.0 CPU 推理环境'});await runNativeCommand(this.python,['-m','pip','install','--index-url','https://pypi.org/simple','piper-tts==1.8.0'],{signal});await runNativeCommand(this.python,['-m','pip','check'],{signal});await writeFile(path.join(this.root,'ready-1.8.0'),'ok');}
