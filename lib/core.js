...
export const schemas = {
  ...
  script: {scenes: [{title: '场次 / 地点 / 时间', action: '人物动作与情绪', dialogue: '人物：台词', shots: [{prompt: '完整主体外观、环境、动作、运镜、光线、音效与对白', duration: 5}]}]},
...
export function messages(stage, project) {
  ...
  return [
    ...
    {role: 'user', content: JSON.stringify({brief: project.brief, bible: project.bible, characters:project.characters, worldbook:project.worldbook, outline: stage !== 'outline' ? project.outline : undefined, script: stage === 'review' ? project.script : undefined})}
  ];
}
...
export function validateSettings(s) {
  ...
  requireValue(['comfy', 'minimax', 'native','seedance','kling','veo'].includes(s.video.provider), '不支持的视频服务');
  if(['seedance','kling','veo'].includes(s.video.provider)){validateCloud(s.video);return s;}
