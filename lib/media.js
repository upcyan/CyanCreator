...
export async function render(timeline, assets, folder, output, signal, audioTracks=[], preset={width:1280,height:720}, subtitles=[]) {
  ...
  // 交叉溶解段校验：fade 片段尾部与下一片段重叠 0.5s
  requireValue(timeline.length > 0 && timeline.length <= 200, '时间线需要 1–200 个片段');
  for (const [i,clip] of timeline.entries()) if (clip.transition==='fade') requireValue(i<timeline.length-1,'转场不能用于最后一个片段');
  const parts = [];
  for (const [index, clip] of timeline.entries()) {
    ...
    const fade = clip.transition==='fade'&&index<timeline.length-1;
  ...
  const fades = timeline.slice(0,-1).map(c=>c.transition==='fade');
  ...
  if (!needsFilter) {
    ...
    requireValue(!fades.some(Boolean)||fades.every(Boolean), '转场目前需用于所有片段衔接处（统一交叉溶解或统一硬切）');
    ...
    if (fades.some(Boolean)) {
      ...
      for (let i=1;i<parts.length;i++) {
        ...
        chain.push(`${prev}[${i}:v]xfade=transition=fade:duration=${FADE}:offset=${off.toFixed(3)}[${out}]`);
        prev=`[${out}]`;
      }
    ...
    await run(process.env.FFMPEG_PATH || ffmpeg, ['-y','-nostdin','-v','error',...inputs,'-filter_complex',expr,'-map',`[${label}]`,'-map',`${parts.length-1}:a?`,'-c:v','libx264','-preset','veryfast','-crf','20','-pix_fmt','yuv420p','-c:a','aac','-ar','48000','-ac','2','-movflags','+faststart', audioTracks.length?path.join(folder,'base.mp4'):output], signal);
  }
