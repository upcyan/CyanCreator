...
export async function render(timeline, assets, folder, output, signal) {
  ...
  await run(process.env.FFMPEG_PATH || ffmpeg, ['-y', '-nostdin', '-v', 'error', '-f', 'concat', '-safe', '1', '-i', manifest, '-c', 'copy', '-movflags', '+faststart', output], signal);
