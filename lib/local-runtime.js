...
export function deploymentDefaults(data, desktop=path.join(process.env.LOCALAPPDATA||'', 'Comfy-Desktop')) {
  const install=path.join(desktop,'ComfyUI-Installs','ComfyUI');
  const root=path.join(install,'ComfyUI'), python=path.join(root,'.venv','Scripts','python.exe'), shared=path.join(desktop,'ComfyUI-Shared','models');
...
export function validateDeploymentConfig(c) {
  for(const key of ['comfyRoot','comfyPython','comfyModels'])if(typeof c[key]!=='string'||c[key].length>2000||(c[key]&&!path.isAbsolute(c[key])))throw new Error('运行环境路径必须是本机绝对路径');
  ...
  const nativePython=c.nativePython??((process.env.LOCALAPPDATA||'')?path.join(process.env.LOCALAPPDATA, 'Programs','Python','Python312','python.exe'):'');
  if(typeof nativePython!=='string'||(nativePython&&!path.isAbsolute(nativePython)))throw new Error('独立 Python 须为绝对路径');
  const nativeDevice=c.nativeDevice??'cuda';if(!['cuda','cpu'].includes(nativeDevice))throw new Error('原生设备无效');
  return {...Object.fromEntries(['comfyRoot','comfyPython','comfyModels','comfyUrl','gpuLayers','contextSize'].map(k=>[k,c[k]])),nativePython,nativeDevice};
}
...
export class LocalRuntime {
  ...
  stop(id){const p=this.processes.get(id);if(!p)throw new Error('该服务不是由当前工作台启动的，请从它的原始服务管理界面停止');p.kill();this.processes.delete(id);}
  async launch(id,exe,args,cwd,url,check,signal) {
    ...
    try {
      for(let i=0;i<240;i++){
        signal.throwIfAborted();if(error||child.exitCode!==null)throw new Error('模型服务启动失败，请检查 runtime-logs/'+id+'.log');
  ...
  async startText(entry,file,config,signal,onProgress) {
    ...
    if(!existsSync(path.join(target,'.ready'))){
      ...
      if(!await findExe(staging))throw new Error('运行环境归档缺少 llama-server.exe');
  ...
  async ensureVideo(config,signal) {
    ...
    if(!existsSync(config.comfyPython)||!existsSync(path.join(config.comfyRoot,'main.py')))throw new Error('未发现 ComfyUI 运行环境，请设置已安装的 ComfyUI 根目录与 Python 路径');
    try {await run(config.comfyPython,['-c','import torch'],signal);}
    catch(e){signal.throwIfAborted();throw new Error('所选 ComfyUI Python 无法加载 PyTorch，请选择 ComfyUI/.venv/Scripts/python.exe 等已安装推理依赖的环境；不要选择 standalone-env 引导环境。');}
