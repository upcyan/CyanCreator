...
export class LocalRuntime {
  ...
  async startText(entry,file,config,signal,onProgress) {
    ...
    await this.launch(entry.id,exe,['--model',file,'--alias',entry.id,'--host','127.0.0.1','--port',String(entry.port),'--ctx-size',String(config.contextSize),'--n-gpu-layers',String(config.gpuLayers),'--jinja'],path.dirname(exe),base+'/v1/models',data=>data.data?.some(m=>m.id===entry.id),signal);
