...
export async function request(base, path, options = {}) {
  ...
  let r;
  try {r = await fetch(base.replace(/\/$/, '') + path, {...options, redirect: 'error', signal: options.signal ? AbortSignal.any([options.signal, AbortSignal.timeout(180000)]) : AbortSignal.timeout(15000)});}
  catch(e) {
    if(options.signal?.aborted)throw e;
    const code=e.cause?.code||e.code||e.name, origin=new URL(base).origin;
    const hint=code==='ECONNREFUSED'?'服务未启动或端口不正确；请启动对应运行时并检查服务地址':code==='ENOTFOUND'||code==='EAI_AGAIN'?'域名解析失败，请检查网络和服务地址':/TIMEOUT|Timeout/i.test(code)?'连接超时，请检查服务状态和网络':'请检查运行时日志、网络与服务地址';
...
function headers(config) {
  const key = config.keyEnv ? process.env[config.keyEnv] : '';
  requireValue(!config.keyEnv || key, `环境变量 ${config.keyEnv} 未设置`);
  return {'Content-Type': 'application/json', ...(key ? {Authorization: `Bearer ${key}`} : {})};
}
export async function generateText(stage, project, config, signal) {
  requireValue(config.model.trim(), '请先在模型中心配置文本模型名称');
  const r = await request(config.baseUrl, '/chat/completions', {method: 'POST', headers: headers(config), signal,
    body: JSON.stringify({model: config.model, messages: messages(stage, project), temperature: config.temperature, max_tokens: config.maxTokens, response_format: {type: 'json_object'}})});
...
export async function minimaxVideo(config, prompt, duration, signal, onRemote) {
  ...
  const task = check(await (await request(config.baseUrl, '/video_generation', {method: 'POST', headers: h, signal, body: JSON.stringify({model: config.model, prompt, duration, resolution: '768P'})})).json());
