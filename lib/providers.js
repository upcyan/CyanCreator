  ...
  for (const type of new Set(Object.values(graph).map(n => n.class_type))) {
    ...
    for (const node of Object.values(graph).filter(n => n.class_type === type)) {
      for (const [name, value] of Object.entries(node.inputs)) {
        if (type === 'LoadImage' && name === 'image') continue; // 图片名由首帧注入，预检不校验示例名
...
export async function comfyVideo(config, prompt, signal, onRemote, imageName=null) {
  const cfg=structuredClone(config);
  if(imageName){const targets=Object.entries(cfg.bindings).filter(([k,b])=>k==='image');requireValue(targets.length,'当前工作流未绑定 image 参数（请使用图生视频模板）');cfg.params.image=imageName;}
...
export async function minimaxVideo(config, prompt, duration, signal, onRemote, image=null, tail=null) {
  requireValue(!tail, 'MiniMax 暂不支持尾帧图；请清除尾帧或改用 Seedance / 可灵');
