...
export async function request(base, path, options = {}) {
  ...
  const r = await fetch(base.replace(/\/$/, '') + path, {...options, redirect: 'error', signal: options.signal ? AbortSignal.any([options.signal, AbortSignal.timeout(180000)]) : AbortSignal.timeout(15000)});
