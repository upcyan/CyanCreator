...
export const cloudTemplates=[
  ...
  {id:'agnes',name:'Agnes · 文生图生视频',provider:'agnes',baseUrl:'https://api.agnes.ai/v1',model:'agnes-video-v2.0',keyEnv:'AGNES_API_KEY',source:'https://agnes.ai/docs/video',params:{ratio:'16:9',resolution:'720p'}}
];
...
export function validateCloud(c){validateEndpoint(c.baseUrl);validateEnv(c.keyEnv);if(c.provider==='kling')validateEnv(c.secretEnv);requireValue(typeof c.model==='string'&&/^[\w.-]{1,160}$/.test(c.model),'模型 ID 无效');requireValue(['16:9','9:16','1:1'].includes(c.params?.ratio),'画幅无效');if(c.provider==='kling')requireValue(['std','pro'].includes(c.params.mode),'可灵模式无效');else requireValue((c.provider==='veo'?['720p','1080p']:['480p','720p','1080p']).includes(c.params.resolution),'分辨率无效');if(c.provider==='veo')requireValue(c.params.ratio!=='1:1','Veo 不支持方形画幅');if(c.provider==='agnes')requireValue(['480p','720p','1080p'].includes(c.params.resolution),'Agnes 分辨率无效');return c;}
export function cloudDuration(provider,duration){requireValue(Number.isInteger(duration),'云视频秒数须为整数');requireValue(provider==='kling'?[5,10].includes(duration):provider==='veo'?[4,6,8].includes(duration):provider==='agnes'?[4,6,8,10].includes(duration):duration>=4&&duration<=12,provider==='kling'?'可灵模板支持 5/10 秒':provider==='veo'?'Veo 模板支持 4/6/8 秒':provider==='agnes'?'Agnes 模板支持 4/6/8/10 秒':'Seedance 模板支持 4–12 秒');}
...
export async function cloudVideo(c,prompt,duration,signal,onRemote,wait=delay,image=null){
  ...
  const post=c.provider==='seedance'?['/contents/generations/tasks',{model:c.model,content:image?[{type:'text',text:prompt},{type:'image',role:'first_frame',image_url:`data:${image.mime};base64,${b64}`}]:[{type:'text',text:prompt}],duration,ratio:c.params.ratio,resolution:c.params.resolution}]:c.provider==='kling'?['/videos/'+(image?'image2video':'text2video'),{model_name:c.model,prompt,mode:c.params.mode||'std',duration:String(duration),aspect_ratio:c.params.ratio,...(image?{image:`data:${image.mime};base64,${b64}`}:{})}]:c.provider==='agnes'?['/videos/generations',{model:c.model,prompt,duration,resolution:c.params.resolution,ratio:c.params.ratio,...(image?{first_frame_image:`data:${image.mime};base64,${b64}`}:{})}]:['/models/'+encodeURIComponent(c.model)+':predictLongRunning',{instances:[{prompt},...(image?[{image:{bytesBase64Encoded:b64,mimeType:image.mime}}]:[])],parameters:{aspectRatio:c.params.ratio,durationSeconds:duration,resolution:c.params.resolution}}];
  const submitted=await(await request(c.baseUrl,post[0],{method:'POST',headers,signal,body:JSON.stringify(post[1])})).json();
  if(c.provider==='agnes')requireValue(!submitted.error&&submitted.task_id,'Agnes 提交失败：'+(submitted.error?.message||submitted.error||'未返回 task_id'));
  const id=c.provider==='seedance'?submitted.id:c.provider==='kling'?submitted.data?.task_id:c.provider==='agnes'?submitted.task_id:submitted.name;requireValue(typeof id==='string'&&id.length<500,'云服务未返回任务 ID，请检查模型权限和参数');onRemote(id);
  const statusPath=c.provider==='seedance'?'/contents/generations/tasks/'+encodeURIComponent(id):c.provider==='kling'?'/videos/'+(image?'image2video':'text2video')+'/'+encodeURIComponent(id):c.provider==='agnes'?'/videos/generations/'+encodeURIComponent(id):'/'+id.split('/').map(encodeURIComponent).join('/');
  for(let i=0;i<720;i++){
    const result=await(await request(c.baseUrl,statusPath,{headers:auth(c),signal})).json();
    const status=c.provider==='kling'?result.data?.task_status:c.provider==='agnes'?result.data?.status||result.status:result.status;
    requireValue(!result.error&&!['failed','cancelled','expired'].includes(status)&&!(result.code&&c.provider==='kling'),'云端视频生成失败，请在服务商控制台查看任务 '+id);
    let url=c.provider==='seedance'&&status==='succeeded'?result.content?.video_url:c.provider==='kling'&&status==='succeed'?result.data?.task_result?.videos?.[0]?.url:c.provider==='agnes'&&status==='succeeded'?(result.data?.video_url||result.video_url):c.provider==='veo'&&result.done?result.response?.generateVideoResponse?.generatedSamples?.[0]?.video?.uri:null;
    ...
    if(c.provider==='agnes'&&status==='failed')throw new Error('Agnes 视频生成失败：'+(result.data?.error||result.error?.message||'未知错误'));
