...
export async function cloudVideo(c,prompt,duration,signal,onRemote,wait=delay){
  validateCloud(c);cloudDuration(c.provider,duration);const headers={'Content-Type':'application/json',...auth(c)};
  const post=c.provider==='seedance'?['/contents/generations/tasks',{model:c.model,content:[{type:'text',text:prompt}],duration,ratio:c.params.ratio,resolution:c.params.resolution}]:c.provider==='kling'?['/videos/text2video',{model_name:c.model,prompt,mode:c.params.mode||'std',duration:String(duration),aspect_ratio:c.params.ratio}]:['/models/'+encodeURIComponent(c.model)+':predictLongRunning',{instances:[{prompt}],parameters:{aspectRatio:c.params.ratio,durationSeconds:duration,resolution:c.params.resolution}}];
  ...
  const statusPath=c.provider==='seedance'?'/contents/generations/tasks/'+encodeURIComponent(id):c.provider==='kling'?'/videos/text2video/'+encodeURIComponent(id):'/'+id.split('/').map(encodeURIComponent).join('/');
  for(let i=0;i<720;i++){
