...
export async function cloudVideo(c,prompt,duration,signal,onRemote,wait=delay,image=null,tail=null){
  validateCloud(c);cloudDuration(c.provider,duration);const headers={'Content-Type':'application/json',...auth(c)};
  const b64=image?image.bytes.toString('base64'):'',tb64=tail?tail.bytes.toString('base64'):'';
  const post=c.provider==='seedance'?['/contents/generations/tasks',{model:c.model,content:(image||tail)?[{type:'text',text:prompt},...(image?[{type:'image',role:'first_frame',image_url:`data:${image.mime};base64,${b64}`}]:[]),...(tail?[{type:'image',role:'last_frame',image_url:`data:${tail.mime};base64,${tb64}`}]:[])]:[{type:'text',text:prompt}],duration,ratio:c.params.ratio,resolution:c.params.resolution}]:c.provider==='kling'?['/videos/'+((image||tail)?'image2video':'text2video'),{model_name:c.model,prompt,mode:c.params.mode||'std',duration:String(duration),aspect_ratio:c.params.ratio,...(image?{image:`data:${image.mime};base64,${b64}`}:{}),...(tail?{image_tail:`data:${tail.mime};base64,${tb64}`}:{})}]:['/models/'+encodeURIComponent(c.model)+':predictLongRunning',{instances:[{prompt},...(image?[{image:{bytesBase64Encoded:b64,mimeType:image.mime}}]:[])],parameters:{aspectRatio:c.params.ratio,durationSeconds:duration,resolution:c.params.resolution}}];
  ...
  const statusPath=c.provider==='seedance'?'/contents/generations/tasks/'+encodeURIComponent(id):c.provider==='kling'?'/videos/'+((image||tail)?'image2video':'text2video')+'/'+encodeURIComponent(id):'/'+id.split('/').map(encodeURIComponent).join('/');
  for(let i=0;i<720;i++){
