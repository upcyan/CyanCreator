...
export function secretValue(name){const value=values[name]??process.env[name]??'';if(value)usedSecrets.add(value);return value;}
export function secretStatus(){return Object.keys(values).map(name=>({name,configured:true}));}
export function safeError(error){let text=String(error?.message||error);for(const value of new Set([...Object.values(values),...usedSecrets]))if(value)text=text.split(value).join('[已隐藏密钥]');return text;}
