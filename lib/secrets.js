...
export function vaultBackend(){return {name:backend,note:backendNote||'',label:backend==='win-dpapi'?'Windows DPAPI':backend==='keyring'?'系统钥匙串':backend==='env-master'?'环境变量主密钥':'不可用'};}
...
async function protect(mode,value){ // value: Buffer / string → Buffer
  ...
  if(backend==='win-dpapi')return winProtect(mode,Buffer.from(value).toString('base64')).then(b=>b);
...
export class SecretVault{
  ...
  async load(){
    let probe=false;
    // 探测后端：文件存在但后端不可用时延迟报错（保持工作台可启动）
    await detectBackend();
    let blob;try{blob=await readFile(this.file);}catch(e){if(e.code==='ENOENT')return;throw e;}
    try{values=JSON.parse((await protect('decrypt',blob)).toString('utf8'));}
    catch(e){backendNote=(backendNote||'')+'；解密失败：'+e.message;throw new Error('密钥保险库无法在本机解密（'+this.backendLabel()+'）：'+e.message);}
  }
